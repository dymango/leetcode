...
public class NumSubarrayProductLessThanK_713 {
     ...
     * 著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。
     */
   static Map<Integer, Map<Integer, Integer>> record = new HashMap<>();
    ...
    private static int windowValue(int[] nums, int s, int e, int k) {
        if(s > e) return 1;
        if(s == e) return nums[s];
        Map<Integer, Integer> map = record.get(s);
        Integer target = 0;
        if(map != null && (target = map.get(e)) != null) return target;
        if(map == null) record.put(s, new HashMap<>());
        int sum = 1;
        for (int i = s; i <= e; i++) {
            sum *= nums[i];
            if(sum > k) break;
